"""
This file is unchanged version of original fastr.py of FAST-R 
present at https://github.com/ICSE19-FAST-R/FAST-R/blob/master/py/fastr.py
"""
...
def fast_pw(input_file, r, b, bbox=False, k=5, memory=False, B=0):
    ...
    if memory:
        ...
        tcs_minhashes = {
            tc[0]: lsh.tcMinhashing(tc, hashes) for tc in test_suite.items()
        }
    ...
    while len(tcs_minhashes) > 0:
        ...
        if iteration % 100 == 0:
            sys.stdout.write(
                "  Progress: {}%\r".format(round(100 * iteration / total, 2))
            )
            sys.stdout.flush()
...
def reductionCS(TS, B):
    ...
    reducedTS = list(
        np.random.choice(list(range(1, len(TS) + 1)), size=B, p=P, replace=False)
    )
