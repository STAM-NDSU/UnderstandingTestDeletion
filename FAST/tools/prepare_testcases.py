...
import os
from deltest.utils.utils import is_candidate_test_file, strip_commit_url
import json
...
for project in PROJECTS:
    ...
    REPO_PATH = f"{deltest_PROJECTS_DIR}/{project}"
    redundant_deleted_tests_filepath = Path(f"{REDUNDANT_TESTS_DIR}/{project}.csv")
    if not os.path.exists(f"{REPO_PATH}"):
        ...
        break
    if not os.path.exists(f"{redundant_deleted_tests_filepath}"):
        print(
            "Error: path does not exit -> ",
            redundant_deleted_tests_filepath,
        )
        break

    redundant_deleted_tc_df = pd.read_csv(redundant_deleted_tests_filepath)
    stripped_parent_commits = redundant_deleted_tc_df["Parent"].map(strip_commit_url)
