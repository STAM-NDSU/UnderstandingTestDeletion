...
class AnalyzerGlobal:
    ...
    def __init__(self):
        self.since = COMMIT_START_DATETIME
        self.to = COMMIT_END_DATETIME
        self.commits = None
...
def get_removed_test_functions_details(
    ...
    def traverse_commits(analyzer_global):
        ...
        commits = Repository(
            ...
            only_modifications_with_file_types=JAVA_FILE_EXT,
            since=since,
            to=to,
            only_no_merge=True,
            # single="<commit_hash>",  # use it only for debugging
        ).traverse_commits()
            ...
            if commit:
                ...
                if len(commit.parents):
                    commit_parent = parse_commit_as_hyperlink(
                        label=commit.parents[0],
                        url=get_full_commit_url(commit.parents[0]),
                    )
                else:
                    commit_parent = ""
